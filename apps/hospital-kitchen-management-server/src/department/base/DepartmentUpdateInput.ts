/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
  IsInt,
  Min,
  Max,
} from "class-validator";
import { IngredientUpdateManyWithoutDepartmentsInput } from "./IngredientUpdateManyWithoutDepartmentsInput";
import { Type } from "class-transformer";
import { InventoryUpdateManyWithoutDepartmentsInput } from "./InventoryUpdateManyWithoutDepartmentsInput";
import { MealUpdateManyWithoutDepartmentsInput } from "./MealUpdateManyWithoutDepartmentsInput";

@InputType()
class DepartmentUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  departmentName?: string | null;

  @ApiProperty({
    required: false,
    type: () => IngredientUpdateManyWithoutDepartmentsInput,
  })
  @ValidateNested()
  @Type(() => IngredientUpdateManyWithoutDepartmentsInput)
  @IsOptional()
  @Field(() => IngredientUpdateManyWithoutDepartmentsInput, {
    nullable: true,
  })
  ingredients?: IngredientUpdateManyWithoutDepartmentsInput;

  @ApiProperty({
    required: false,
    type: () => InventoryUpdateManyWithoutDepartmentsInput,
  })
  @ValidateNested()
  @Type(() => InventoryUpdateManyWithoutDepartmentsInput)
  @IsOptional()
  @Field(() => InventoryUpdateManyWithoutDepartmentsInput, {
    nullable: true,
  })
  inventories?: InventoryUpdateManyWithoutDepartmentsInput;

  @ApiProperty({
    required: false,
    type: () => MealUpdateManyWithoutDepartmentsInput,
  })
  @ValidateNested()
  @Type(() => MealUpdateManyWithoutDepartmentsInput)
  @IsOptional()
  @Field(() => MealUpdateManyWithoutDepartmentsInput, {
    nullable: true,
  })
  meals?: MealUpdateManyWithoutDepartmentsInput;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  totalRequiredItems?: number | null;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  totalRequiredMaterials?: number | null;
}

export { DepartmentUpdateInput as DepartmentUpdateInput };
